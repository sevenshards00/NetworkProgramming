...
* 작성 일자: 2023-12-27
* 이전 버전 작성 일자:
* 버전 내용: epoll 기반의 echo 서버 - Edge Trigger
* 이전 버전 내용:
*/
...
#include <sys/epoll.h>
// Edge Trigger 모델을 사용할 경우 필요한 헤더
#include <errno.h>
#include <fcntl.h>

#define BUF_SIZE 4
#define EPOLL_SIZE 50
void setnonblockingmode(int fd); // Non-blocking IO를 위한 소켓 모드 변경 함수
...
int main(int argc, char* argv[])
{
    ...
    setnonblockingmode(serv_sock); // Edge Trigger 모델에서 추가된 부분 -> 소켓을 Non-blocking 모드로
    ...
    while(1)
    {
        ...
        for(i = 0; i < event_cnt; i++)
        {
            ...
            if(ep_events[i].data.fd == serv_sock)
            {
                ...
                {
                    setnonblockingmode(clnt_sock); // Edge Trigger 모델에서 추가된 부분 -> 소켓을 Non-blocking 모드로
                    puts("New Client Connected....");
                    // 새로 생성된 클라이언트와 연결된 소켓(파일 디스크립터)를 epoll 인스턴스에 추가로 등록한다
                    // Edge Trigger 기반으로 동작하기 위해 EPOLLET을 추가
            ...
            else
            {
                while(1)
                {
                    ...
                    else if(str_len < 0) // 입력 버퍼가 정말 비어있는지 확인
                    {
                        if(errno == EAGAIN) // 입력 버퍼가 비어있는 상황에서 에러코드가 EAGAIN이라면
                            break; // 더 읽어들일 데이터가 없으므로 break
                    }
                    else // 수신된 데이터가 있는 상황
