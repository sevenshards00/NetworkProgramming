...
* 작성 일자: 2023-12-27
* 이전 버전 작성 일자:
* 버전 내용: 입출력 스트림의 분리 이후 half-close에 대한 문제 해결 - dup을 이용한 파일 디스크립터 복사
* 이전 버전 내용:
*/
...
int main(int argc, char* argv[])
{
	...
	writefp = fdopen(dup(clnt_sock), "w"); // 파일 디스크립터 clnt_sock을 복사하여 파일 포인터 생성
    ...
    // 파일 포인터 writefp를 fileno 함수를 통해 다시 파일 디스크립터로 변경
    // 이후 shutdown 함수를 통해 half-close 수행
