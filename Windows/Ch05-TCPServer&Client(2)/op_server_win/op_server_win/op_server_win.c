...
int main(int argc, char* argv[])
{
	...
	SOCKADDR_IN serv_addr, clnt_addr; // 서버와 클라이언트의 주소를 담을 구조체, IPv4 기준
	int clnt_addr_sz; // 클라이언트의 주소 값의 크기
	...
	if (hServSock == INVALID_SOCKET) // 소켓 생성이 안됐을 경우
		ErrorHandling("socket() error");

	ZeroMemory(&serv_addr, sizeof(serv_addr)); // serv_addr 구조체의 메모리를 전부 0으로 초기화
	...
	for (i = 0; i < 5; i++)
	{
		...
		recv(hClntSock, &opnd_cnt, 1, 0); // 첫 바이트만 읽어들인다. 여기서 피연산자 갯수 정보를 가져온다. (Parsing)
