...
* 파일명: SyncMutex_win.cpp
* 파일 버전: 0.1
* 작성자: Sevenshards
* 작성 일자: 2023-12-28
* 이전 버전 작성 일자:
* 버전 내용: 커널 모드 동기화(1) - Mutex 기반의 동기화
* 이전 버전 내용:
*/
...
int main(int argc, char* argv[])
{
	...
	hMutex = CreateMutex(
		NULL, // 보안 속성(상속 여부)
		FALSE, // TRUE일 경우 Mutex를 생성한 쓰레드가 Non-Signaled 상태로 선점, FALSE일 경우 Signaled 상태로 아무도 선점하지 않은 상태
		NULL // Mutex에 이름을 부여할 때 사용, 필요하지 않다면 NULL
	);
...
{
	WaitForSingleObject(hMutex, INFINITE); // Signaled -> Non-Signaled
